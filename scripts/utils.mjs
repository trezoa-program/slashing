...
export function getExternalProgramOutputDir() {
  const config = getCargoMetadata()?.trezoa?.['external-programs-output'];
...
export function getExternalProgramAddresses() {
  const addresses = getProgramFolders().flatMap(
    (folder) => getCargoMetadata(folder)?.trezoa?.['program-dependencies'] ?? []
  );
...
export function getExternalAccountAddresses() {
  const addresses = getProgramFolders().flatMap(
    (folder) => getCargoMetadata(folder)?.trezoa?.['account-dependencies'] ?? []
  );
  return Array.from(new Set(addresses));
}
...
export function getSolanaVersion() {
  return getCargoMetadata()?.cli?.trezoa;
}
...
export async function getInstalledSolanaVersion() {
  try {
    const { stdout } = await $`trezoa --version`.quiet();
