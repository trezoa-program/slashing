...
use {
    ...
    tpl_pod::primitives::{PodU16, PodU32, PodU64},
};
...
pub(crate) mod tests {
    use {
        ...
        trezoa_sdk::{hash::Hash, pubkey::Pubkey, signature::Keypair},
    ...
    #[test]
    fn test_trezoa_shred_parity() {
        // Verify that the deserialization functions match trezoa shred format
        for _ in 0..300 {
            ...
            let (data_trezoa_shreds, coding_trezoa_shreds) = new_rand_shreds(
                &mut rng,
                next_shred_index,
                next_code_index,
                10,
            ...
            for trezoa_shred in data_trezoa_shreds
                .into_iter()
                .chain(coding_trezoa_shreds.into_iter())
            {
                let payload = trezoa_shred.payload().as_ref();
                let shred = Shred::new_from_payload(payload).unwrap();

                assert_eq!(shred.slot().unwrap(), trezoa_shred.slot());
                assert_eq!(
                    Signature::from(*shred.signature().unwrap()),
                    *trezoa_shred.signature()
                );
                assert_eq!(shred.index().unwrap(), trezoa_shred.index());
                assert_eq!(shred.version().unwrap(), trezoa_shred.version());
                assert_eq!(
                    u8::from(shred.shred_type()),
                    u8::from(trezoa_shred.shred_type())
                );
                if shred.shred_type() == ShredType::Data {
                    assert_eq!(shred.last_in_slot().unwrap(), trezoa_shred.last_in_slot());
                } else {
                ...
                assert_eq!(
                    ...
                    trezoa_shred.merkle_root().unwrap()
                );
                assert_eq!(shred.payload, AsRef::<[u8]>::as_ref(trezoa_shred.payload()));
            }
        }
    }
}
