use {
    crate::cli::*,
    clap::ArgMatches,
    trezoa_clap_v3_utils::keypair::{signer_from_path, signer_from_source},
    ...
    trezoa_sdk::{commitment_config::CommitmentConfig, signature::Signer},
...
impl Config {
    pub async fn new(
        ...
        let cli_config = if let Some(config_file) = &cli.config_file {
            trezoa_cli_config::Config::load(config_file).unwrap_or_else(|_| {
                eprintln!("error: Could not load config file `{}`", config_file);
                exit(1);
            })
        } else if let Some(config_file) = &*trezoa_cli_config::CONFIG_FILE {
    ...
    pub fn default_signer(&self) -> Result<Arc<dyn Signer>, Error> {
        if let Some(default_signer) = &self.default_signer {
            ...
            Err("default signer is required, please specify a valid default signer by identifying a \
                 ...
                 at the default location of ~/.config/trezoa/cli/config.yml using the trezoa config \
    ...
    pub fn fee_payer(&self) -> Result<Arc<dyn Signer>, Error> {
        if let Some(fee_payer) = &self.fee_payer {
            ...
            Err("fee payer is required, please specify a valid fee payer using the --payer argument, or \
                 ...
                 valid config at the default location of ~/.config/trezoa/cli/config.yml using the trezoa \
